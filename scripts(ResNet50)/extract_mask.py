...
from FgSegNet_v2_Resnet import loss, acc, loss2, acc2
...
def checkFrame(X_list):
    ...
    max_frames = 400 # max frames to slice
    if(img[1]>=300 and len(X_list)>max_frames):
...
raw_dataset_dir = '/content/FgSegNet_v2/datasets/CDnet2014_dataset/'

# 2. model dir
main_mdl_dir = '/content/FgSegNet_v2/FgSegNet_v2/CDnet/models200/'
...
for category, scene_list in dataset.items():
    ...
    for scene in scene_list:
        ...
        model = load_model(mdl_path, custom_objects={'MyUpSampling2D': MyUpSampling2D, 'InstanceNormalization': InstanceNormalization, 'loss':loss, 'acc':acc, 'loss2':loss2, 'acc2':acc2})
